// Derived from DevilutionX's Source/mpq/mpq_writer.cpp
...
void ByteSwapHdr(MpqFileHeader *hdr)
{
    ...
    hdr->blockSizeFactor = SwapLE16(hdr->blockSizeFactor);
    hdr->hashEntriesOffset = SwapLE32(hdr->hashEntriesOffset);
    hdr->blockEntriesOffset = SwapLE32(hdr->blockEntriesOffset);
    hdr->hashEntriesCount = SwapLE32(hdr->hashEntriesCount);
    hdr->blockEntriesCount = SwapLE32(hdr->blockEntriesCount);
}
...
MpqWriter::MpqWriter(const char *path)
{
	...
	return;
on_error:
        app_fatal(StrCat("Failed to open archive for writing.\n", path, "\n", error));
}
...
bool MpqWriter::WriteFileContents(const std::byte *fileData, uint32_t fileSize, MpqBlockEntry *block)
{
	...
	while (true) {
		...
		if (!stream_.Write(reinterpret_cast<const char *>(&mpqBuf[0]), len))
			return false;
           offsetTable[curSector++] = SwapLE32(destSize);
    ...
    offsetTable[numSectors] = SwapLE32(destSize);
